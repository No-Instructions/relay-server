...
fn parse_private_key_format(input: &str) -> Result<AuthKeyMaterial, AuthError> {
    ...
    if trimmed.starts_with("-----BEGIN") && trimmed.contains("-----END") {
            ...
            || trimmed.contains("-----BEGIN EC PRIVATE KEY-----")
        {
            ...
            tracing::error!("Unsupported PEM private key type");
            Err(AuthError::InvalidToken)
        }
    } else {
        // Raw base64 - distinguish between legacy (30 bytes), HMAC256 (32 bytes), and Ed25519 (32 bytes)
        let key_bytes = b64_decode(trimmed)?;
        tracing::info!("Parsing private key ({} bytes)", key_bytes.len());
        ...
        match key_bytes.len() {
            ...
            32 => {
                // For private keys, 32 bytes could be either HMAC or Ed25519
                // Default to HMAC for backward compatibility, but in the future we could
                // add detection logic or require explicit specification
                tracing::info!("Using 32-byte private key for HMAC-SHA256 (CWT tokens)");
                Ok(AuthKeyMaterial::Hmac256(key_bytes))
            }
            _ => {
                tracing::warn!(
                    "Unexpected private key length: {} bytes, defaulting to HMAC256",
                    key_bytes.len()
                );
                Ok(AuthKeyMaterial::Hmac256(key_bytes))
            }
...
fn parse_public_key_format(input: &str) -> Result<AuthKeyMaterial, AuthError> {
    ...
    if trimmed.starts_with("-----BEGIN") && trimmed.contains("-----END") {
        ...
        if trimmed.contains("-----BEGIN PUBLIC KEY-----") {
            // Could be ECDSA or Ed25519 public key in PEM format
            let key_bytes = extract_pem_content(trimmed)?;
            // Try to determine if it's Ed25519 or ECDSA based on length
            match key_bytes.len() {
                32 => {
                    tracing::info!("Parsed PEM Ed25519 public key");
                    Ok(AuthKeyMaterial::Ed25519Public(key_bytes))
                }
                _ => {
            ...
            tracing::error!("Unsupported PEM public key type");
            Err(AuthError::InvalidToken)
        }
    } else {
        // Raw base64 - distinguish key types by length
        let key_bytes = b64_decode(trimmed)?;
        tracing::info!("Parsing public key ({} bytes)", key_bytes.len());

        match key_bytes.len() {
            32 => {
                // For public keys, 32 bytes should be Ed25519, not HMAC
                tracing::info!("Using 32-byte key as Ed25519 public key");
                Ok(AuthKeyMaterial::Ed25519Public(key_bytes))
            }
            33 => {
                tracing::info!("Using 33-byte key as ECDSA P-256 compressed public key");
                Ok(AuthKeyMaterial::EcdsaP256Public(key_bytes))
            }
            65 => {
                tracing::info!("Using 65-byte key as ECDSA P-256 uncompressed public key");
                Ok(AuthKeyMaterial::EcdsaP256Public(key_bytes))
            }
            _ => {
                tracing::error!("Unsupported public key length: {} bytes", key_bytes.len());
                Err(AuthError::InvalidToken)
            }
        }
    }
...
// Keep the old function for backward compatibility, defaulting to private key behavior
fn parse_key_format(input: &str) -> Result<AuthKeyMaterial, AuthError> {
    parse_private_key_format(input)
}
...
mod tests {
    ...
    fn test_multi_key_token_verification_with_key_id() {
        ...
        let configs = vec![
            ...
            AuthKeyConfig {
                key_id: Some("hmac-public-key".to_string()),
                private_key: None,
                public_key: Some(b64_encode(&[1u8; 32])), // Use 32-byte Ed25519 public key
            },
        ];
    ...
    fn test_multi_key_configuration_ordering() {
        ...
        let configs = vec![
            ...
            AuthKeyConfig {
                ...
                private_key: None,
                public_key: Some(b64_encode(&[2u8; 32])), // Use 32-byte Ed25519 public key
            },
        ];
    ...
    #[test]
    fn test_public_key_vs_private_key_parsing() {
        use crate::config::AuthKeyConfig;

        // Test 32-byte key specified as public_key should be Ed25519, not HMAC
        let ed25519_public_key_b64 = "8+z0eGUEMkOq8feWi0u035bW7/3iszl21nVx8er3d6A="; // 32 bytes

        let public_key_config = vec![AuthKeyConfig {
            key_id: Some("staging".to_string()),
            private_key: None,
            public_key: Some(ed25519_public_key_b64.to_string()),
        }];

        // Should parse successfully as Ed25519 public key
        let authenticator = Authenticator::from_multi_key_config(&public_key_config).unwrap();

        // Verify the key material type is Ed25519Public, not Hmac256
        let key_entry = &authenticator.keys[0];
        assert!(matches!(
            key_entry.key_material,
            AuthKeyMaterial::Ed25519Public(_)
        ));
        assert_eq!(key_entry.can_sign, false); // Public keys cannot sign
        assert_eq!(key_entry.key_id, Some("staging".to_string()));

        // Test 32-byte key specified as private_key should still be HMAC for backward compatibility
        let hmac_private_key_b64 = b64_encode(&[42u8; 32]); // 32 bytes of test data

        let private_key_config = vec![AuthKeyConfig {
            key_id: Some("hmac_key".to_string()),
            private_key: Some(hmac_private_key_b64),
            public_key: None,
        }];

        let authenticator = Authenticator::from_multi_key_config(&private_key_config).unwrap();

        // Verify the key material type is still Hmac256 for private keys
        let key_entry = &authenticator.keys[0];
        assert!(matches!(
            key_entry.key_material,
            AuthKeyMaterial::Hmac256(_)
        ));
        assert_eq!(key_entry.can_sign, true); // Private keys can sign
        assert_eq!(key_entry.key_id, Some("hmac_key".to_string()));

        // Test different length public keys
        let ecdsa_compressed_key_b64 = b64_encode(&[1u8; 33]); // 33 bytes - ECDSA compressed
        let ecdsa_uncompressed_key_b64 = b64_encode(&[2u8; 65]); // 65 bytes - ECDSA uncompressed

        let ecdsa_configs = vec![
            AuthKeyConfig {
                key_id: Some("ecdsa_compressed".to_string()),
                private_key: None,
                public_key: Some(ecdsa_compressed_key_b64),
            },
            AuthKeyConfig {
                key_id: Some("ecdsa_uncompressed".to_string()),
                private_key: None,
                public_key: Some(ecdsa_uncompressed_key_b64),
            },
        ];

        let authenticator = Authenticator::from_multi_key_config(&ecdsa_configs).unwrap();

        // Both should be parsed as ECDSA public keys
        assert!(matches!(
            authenticator.keys[0].key_material,
            AuthKeyMaterial::EcdsaP256Public(_)
        ));
        assert!(matches!(
            authenticator.keys[1].key_material,
            AuthKeyMaterial::EcdsaP256Public(_)
        ));
        assert_eq!(authenticator.keys[0].can_sign, false);
        assert_eq!(authenticator.keys[1].can_sign, false);
    }
}
